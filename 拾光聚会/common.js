...
window.Common = {
    ...
    renderBottomNav(activeTabId) {
        const tabs = [
            ...
            { id: 'radio', label: '广播', icon: 'radio', href: 'radio.html' },
            { id: 'discover', label: '发现', icon: 'compass', href: 'discover.html' },
            { id: 'community', label: '社区', icon: 'message-square', href: 'community.html' },
            { id: 'me', label: '我的', icon: 'user', href: 'me.html' },
        ];

        // 检查是否有未读消息
        const hasUnreadMessages = window.GlobalData?.messages?.some(msg => msg.unread > 0) || false;
        ...
        const buttonsHtml = tabs.map(tab => {
            ...
            const showBadge = tab.id === 'me' && hasUnreadMessages;

            return `
            <a href="${tab.href}" class="flex flex-col items-center justify-center w-16 space-y-1 transition-all duration-200 no-underline relative">
              <div class="p-1.5 rounded-full transition-colors ${bgClass} relative">
                <i data-lucide="${tab.icon}" class="w-6 h-6 ${iconClass}" stroke-width="${strokeWidth}"></i>
                ${showBadge ? '<span class="absolute top-0 right-0 w-2 h-2 bg-red-500 rounded-full border border-white"></span>' : ''}
              </div>
